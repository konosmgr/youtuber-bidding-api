...
from .models import Bid, Item, Category, ItemImage, LoginAttempt, BidAttempt, User, Message
from .serializers import (
    BidSerializer, ItemSerializer, CategorySerializer, UserSerializer,
    UserRegistrationSerializer, LoginSerializer, GoogleAuthSerializer, MessageSerializer
)
...
def login_view(request):
    ...
    captcha_required = LoginAttempt.objects.filter(
        email=email,
        ip_address=ip_address,
...
def resend_verification(request):
    ...
    try:
        ...
        if user.verification_token_expires and user.verification_token_expires > timezone.now() - timedelta(hours=23):
            time_remaining = (user.verification_token_expires - (timezone.now() - timedelta(hours=23))).seconds // 60
...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            user = request.user
            print(f"Message creation request from user: {user.username} (is_staff: {user.is_staff})")
...
def winner_ids(request):
    ...
    winner_ids = Item.objects.filter(
        winner__isnull=False,
        end_date__lt=timezone.now()
    ).values_list('winner_id', flat=True).distinct()
...
def mark_winners(request):
    try:
        ...
        if not item_ids or not user_id:
            return Response({"detail": "Both item_ids and user_id are required"}, 
                          status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            # Check if auction has ended
            if item.end_date > timezone.now():
                return Response({"detail": "Cannot assign winner to active auction"}, 
                              status=status.HTTP_400_BAD_REQUEST)
            
            print(f"Assigning user {user.email} (ID: {user.id}) as winner for item {item.title} (ID: {item.id})")
            ...
            return Response({"detail": f"Item with ID {item_id} not found"}, 
                          status=status.HTTP_404_NOT_FOUND)
        except User.DoesNotExist:
